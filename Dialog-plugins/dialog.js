	...
	$.Dialog = function(settings) {
		if (settings.type === "alert") {
			...
			if (settings.button) {
				...
				action = '<div class="ui-dialog-action"><a class="ui-alert-submit  js-dialog-close">' + settings.button + '</a></div>';
		...
		if (settings.type === "confirm") {
			...
			for (var i = 0, l = settings.buttons.length; i < l; i++) {
				...
				if (item.yes) {
					btnstr += '<a class="ui-confirm-submit " data-type="yes">' + item.yes + '</a>';
				}
				if (item.no) {
					btnstr += '<a class="ui-confirm-no" data-type="no">' + item.no + '</a>';
				}
				if (item.close) {
					btnstr += '<a class="ui-confirm-close js-dialog-close" data-type="close">' + item.close + '</a>';
				}
			}
			action = '<div class="ui-dialog-action">' + btnstr + '</div>';
			html += action;
			var options = $.extend({
				target: html,
				animate: true,
